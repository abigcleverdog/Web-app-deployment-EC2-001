...
# Create dummy users
ptext = 'https://pbs.twimg.com/profile_images/2671170543/'
ptext += '18debd694829ed78203a5a36dd364160_400x400.png'
...
item1 = Item(user_id=1, name="Two shinguards",
             description="So you can still walk after the game.",
             category=cat1)
session.add(item1)
session.commit()

item2 = Item(user_id=1, name="Jersey",
             description="Looks cool!", category=cat1)
session.add(item2)
session.commit()

item3 = Item(user_id=1, name="Soccer Cleats",
             description="Perfect for butt-kicking.", category=cat1)
session.add(item3)
session.commit()

# Items for snowboarding
item4 = Item(user_id=2, name="Snowboard",
             description="It's a board.", category=cat5)
session.add(item4)
session.commit()
